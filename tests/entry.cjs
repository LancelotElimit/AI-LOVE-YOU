...
(async()=>{
  ...
  try{
    ...
    const namedSave=await saved(page);await clickControl(page,'home');await page.locator('#title-load').click();await page.locator('.save-slot').nth(1).getByRole('button').click();assert.equal((await saved(page)).state.playerName,'林澈');
    const legacy=structuredClone(namedSave);delete legacy.state.playerName;
    await clickControl(page,'load');await page.locator('#import-file').setInputFiles({name:'legacy.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(legacy))});await page.waitForFunction(()=>!document.querySelector('#modal').open);assert.equal((await saved(page)).state.playerName,'旅人');
    await clickControl(page,'home');await page.locator('#title-new').click();await page.locator('#restart-confirm').click();await goToLogin(page);
    ...
    await clickControl(page,'home');await page.locator('#title-new').click();await page.locator('#restart-confirm').click();assert.equal((await saved(page)).state.playerName,'');
    await goToLogin(page);await clickControl(page,'home');await page.locator('#title-continue').click();assert.ok(await page.locator('#station-screen').isVisible());
