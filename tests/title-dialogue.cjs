...
(async()=>{
  ...
  try {
    ...
    assert.equal(await page.locator('.title-slide').count(),4);
    const first=await page.locator('.title-slide.current').getAttribute('src');
    await page.clock.runFor(10000);assert.match(await page.locator('.title-slide.current').getAttribute('src'),/deepseek/);
    await page.clock.runFor(30000);assert.equal(await page.locator('.title-slide.current').getAttribute('src'),first);
    ...
    await clickControl(page,'home');await page.clock.runFor(10000);assert.match(await page.locator('.title-slide.current').getAttribute('src'),/deepseek/);
    assert.deepEqual(errors,[]);console.log('Four-image 10s loop, pause outside title, dialogue hide/restore without advancing, mobile toolbar: passed.');
