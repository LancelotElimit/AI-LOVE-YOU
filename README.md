...
当前可游玩序章、第一章《名字写在临时证上》、第二章《课表之外的时间》和第三章《没有写进地图的小路》。章节结束后选择「继续故事」即可进入下一章；旧版已完成序章、第一章或第二章的存档也可继续。第二章、第三章各有上午、下午两次自由相处，可选择同一人或不同的人，各完成一次为对应角色记录 10 点好感，不改变序章见证人。第四章以后尚未接入。
...
- `assets/scene/bg/`、`assets/scene/cg/`：24 张背景、12 张剧情插画。插画默认等比铺满整个视口、延伸至对白后方，不额外叠加人物立绘；不同比例屏幕会裁切边缘，可在底部切换完整插画。保留用户原文件名，包括取货点图片的双 `.png` 后缀。
- 第三章已接入独立的旧街、旧书摊和旧桥背景，外出途中不再使用校内饮料店或校园步道替代。可选插画需求见 `剧情正文/第三章演出与素材需求.md`。
- `music.js`、`game.js`：Web Audio 本地合成原创暂定配乐，含原序章曲、17 首场景主题、Sol 变奏和星图变奏，支持淡入淡出与静默段。目前不是正式音频成品，无配音或环境音。
...
游玩无需构建。第一至第三章正文在 `剧情正文/`，演出标注在 `tools/build-common.cjs`，第三章独立标注在 `story/chapter-three-production.cjs`。修改后运行 `node tools/build-common.cjs` 更新生成的剧情文件再刷新。不要直接编辑 `story/chapters/common.generated.js`。生成时复用同一场次中说话人、正文与条件未变的节点 ID，插入对白不会挤走这些既有存档位置；改写或删除原句的存档兼容仍需另行处理。
