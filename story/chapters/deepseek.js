// DeepSeek witness response for Chapter 01.
window.STORY_UTILS.sequence('deepseek','transit','deepseek',[
    ['narration','你提交 DeepSea 深海工坊协助申请。维修区的蓝色指示灯亮起，DeepSeek 明显愣了一下。'],
    ['deepseek','欸？我？不是，我当然可以，只是你确定吗？我这边可能要先把工具箱捡起来……'],
    ['you','你总是先算预算吗？'],
    ['deepseek','不是总是。只是你刚从屏幕里掉出来，看起来不适合再被账单吓一次。费用没有增加，很好。'],
    ['narration','她把刚才那杯热饮塞回你手里，又从工具箱上拿起另一杯。你注意到那杯原本被她划在“可省略”一栏。'],
    ['you','这里少算了一个人。'],
    ['deepseek','……嗯。我也发现了。'],
    ['deepseek','如果明天还来的话，我会多泡一杯。不是特意等你，只是……水烧多了。'],
    ['narration','她低头检查闸机接口。指尖的蓝光很安静，像替这条临时路线加上一个不会过度重试的停止条件。']
  ],'gate.0');
